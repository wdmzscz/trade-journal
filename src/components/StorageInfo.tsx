...
export function StorageInfo({ variant = 'light', className }: { variant?: 'light' | 'dark'; className?: string }) {
  ...
  return (
    <div className={cn('text-xs', isDark ? 'text-slate-500' : 'text-slate-500 dark:text-slate-400', className)}>
      <div className="flex items-center gap-1.5">
        ...
        <p className={cn('font-medium', isDark ? 'text-slate-400' : 'text-slate-600 dark:text-slate-400')}>
