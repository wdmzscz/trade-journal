...
function extractFlexChangeInNav(text: string): IbkrAccountFinancials | null {
  const lines = text.split(/\r?\n/)
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i]
    if (
...
function extractFlexFinancials(text: string): IbkrAccountFinancials | null {
  ...
  return {
    startingCapital: first ? parseNumber(first['Total']) : 0,
    currentCapital: parseNumber(last['Total']),
    totalDeposits: 0,
    totalWithdrawals: 0,
    cashFlows: [],
  }
...
function extractFinancials(rows: string[][]): IbkrAccountFinancials | null {
  ...
  if (startingCapital <= 0 && totalDeposits > 0) {
    startingCapital = totalDeposits
  }
