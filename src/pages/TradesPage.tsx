    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-lg border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-surface-800 dark:bg-surface-800"
        >
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          <div className="relative sm:col-span-2">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-200 dark:border-surface-700 py-2 pl-9 pr-3 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
            />
          </div>
          ...
          <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="rounded-lg border border-slate-200 dark:border-surface-700 px-3 py-2 text-sm" />
          <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="rounded-lg border border-slate-200 dark:border-surface-700 px-3 py-2 text-sm" />
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((trade) => (
                  <tr key={trade.id} className="border-t border-slate-100 dark:border-surface-800 hover:bg-slate-50 dark:hover:bg-surface-800 dark:bg-surface-800">
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{trade.entryDate.slice(0, 10)}</td>
                    <td className="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100">{trade.symbol}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{trade.setup ?? trade.assetClass ?? '-'}</td>
                    <td className="px-4 py-3">
                      <span className={trade.side === 'long' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}>
                    ...
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{trade.setup ?? '-'}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {trade.tags.map((tag) => (
                          <span key={tag} className="rounded bg-brand-50 dark:bg-brand-950/40 px-1.5 py-0.5 text-xs text-brand-700 dark:text-brand-300">{tag}</span>
                        ))}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${trade.status === 'closed' ? 'bg-slate-100 dark:bg-surface-800 text-slate-600 dark:text-slate-400' : 'bg-amber-50 dark:bg-amber-950/30 text-amber-700'}`}>
                      ...
                      {(() => {
                          ...
                          return (
                            <div className="flex items-center gap-1">
                              <a
                                ...
                                className="relative inline-flex items-center gap-1 rounded-lg border border-emerald-200 bg-emerald-50 dark:bg-emerald-950/40 px-2 py-1 text-xs font-medium text-emerald-700 dark:text-emerald-400 transition-colors hover:bg-emerald-100"
                              >
                              ...
                              <button
                                ...
                                title="编辑入场图"
                                className="rounded-lg border border-slate-200 dark:border-surface-700 p-1 text-slate-400 transition-colors hover:border-brand-300 hover:bg-brand-50 dark:bg-brand-950/40 hover:text-brand-600 dark:text-brand-400"
                              >
                        ...
                        return (
                          <button
                            ...
                            onClick={() => setChartTrade(trade)}
                            title="添加入场图"
                            className="inline-flex items-center gap-1 rounded-lg border border-slate-200 dark:border-surface-700 px-2 py-1 text-xs font-medium text-slate-500 dark:text-slate-400 transition-colors hover:border-brand-300 hover:text-brand-600 dark:text-brand-400"
                          >
                            <LineChart className="h-3.5 w-3.5" />
                            添加
                          </button>
...
function Select({ value, onChange, options }: { value: string; onChange: (v: string) => void; options: { value: string; label: string }[] }) {
  return (
    <select
      ...
      className="rounded-lg border border-slate-200 dark:border-surface-700 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none"
    >
      {options.map((o) => (
