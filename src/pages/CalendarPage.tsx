  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 px-3 py-2 shadow-sm">
          <button
            onClick={() => setSelectedYear((y) => y - 1)}
            className="rounded p-1 hover:bg-slate-100 dark:hover:bg-surface-800 dark:bg-surface-800"
            aria-label="上一年"
          >
            <ChevronLeft className="h-4 w-4" />
          </button>
          <span className="min-w-[4rem] text-center font-semibold text-slate-900 dark:text-slate-100">{selectedYear}</span>
          <button
            onClick={() => setSelectedYear((y) => y + 1)}
            className="rounded p-1 hover:bg-slate-100 dark:hover:bg-surface-800 dark:bg-surface-800"
            aria-label="下一年"
          ...
          <div className="text-right">
            {capitalContext.totalDeposits != null && capitalContext.totalDeposits > 0 && (
              <p className="text-xs text-slate-500 dark:text-slate-400">
                累计入金 <span className="font-semibold text-slate-700 dark:text-slate-300">{formatCurrency(capitalContext.totalDeposits)}</span>
              </p>
            )}
            {capitalContext.startingCapital > 0 && capitalContext.startingCapital !== capitalContext.totalDeposits && (
              <p className="text-xs text-slate-500 dark:text-slate-400">
                期初净值 <span className="font-semibold text-slate-700 dark:text-slate-300">{formatCurrency(capitalContext.startingCapital)}</span>
              </p>
            )}
            {capitalContext.currentCapital != null && capitalContext.currentCapital > 0 && (
              <p className="text-xs text-slate-500 dark:text-slate-400">
                当前净资产 <span className="font-semibold text-slate-700 dark:text-slate-300">{formatCurrency(capitalContext.currentCapital)}</span>
              </p>
            )}
            <p className="text-xs text-slate-500 dark:text-slate-400">月度总盈亏</p>
        ...
        <div className="space-y-1">
          {monthWeeks.map((week, wi) => (
            <div key={wi} className="grid grid-cols-8 gap-1">
              ...
              <div className="flex min-h-[5rem] flex-col items-center justify-center rounded-lg bg-slate-50 dark:bg-surface-800 px-1 text-center">
                {week.weekTrades > 0 ? (
                  <>
                    <span className={`text-xs font-semibold ${week.weekPnl >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
      ...
      <div className="rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 p-5 shadow-sm">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300">年度日历</h2>
            ...
            return (
              <button
                ...
                className={`rounded-lg border p-3 text-left transition-colors ${
                  isActive ? 'border-brand-500 bg-brand-50 dark:bg-brand-950/40 ring-1 ring-brand-500' : 'border-slate-200 dark:border-surface-700 hover:border-slate-300 dark:border-surface-600'
                }`}
              >
                ...
                <div className="grid grid-cols-7 gap-0.5">
                  ...
                  {days.map((dateStr) => {
                    ...
                    return (
                      ...
                      >
                        {hasJournal && (
                          <span className="absolute bottom-0 left-1/2 h-0.5 w-0.5 -translate-x-1/2 rounded-full bg-white dark:bg-surface-900" />
                        )}
                      </div>
                    )
                  })}
      ...
      {/* Selected day detail */}
      {selectedDate && (
        <div className="rounded-xl border border-brand-200 dark:border-brand-800 bg-brand-50/50 dark:bg-brand-950/40 p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-slate-100">
                {format(parseISO(selectedDate), 'yyyy年 M月 d日 EEEE', { locale: zhCN })}
              </h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">{selectedDayTrades.length} 笔交易</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              {journalDates.has(selectedDate) && (
                <Link
                  to="/journal"
                  className="flex items-center gap-1 rounded-lg bg-white dark:bg-surface-900 px-3 py-1.5 text-xs font-medium text-brand-600 dark:text-brand-400 shadow-sm hover:bg-brand-50 dark:bg-brand-950/40"
                >
          ...
          {selectedDayTrades.length > 0 && (
            <div className="mt-3 space-y-2">
              {selectedDayTrades.map((t) => (
                <div key={t.id} className="flex items-center justify-between rounded-lg bg-white dark:bg-surface-900 px-3 py-2 text-sm">
          ...
          {monthCumulative.length > 0 ? (
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={monthCumulative}>
                ...
                <XAxis dataKey="date" tick={{ fontSize: 11, fill: chartTick }} tickFormatter={(v) => v.slice(8)} />
      ...
      {monthTrades.length > 0 && (
        <div className="rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 p-5 shadow-sm">
          <h2 className="mb-4 text-sm font-semibold text-slate-700 dark:text-slate-300">本月交易明细</h2>
              ...
              <tbody>
                {monthTrades.map((t) => (
                  <tr key={t.id} className="border-b border-slate-100 dark:border-surface-800">
                    <td className="py-2.5 text-slate-600 dark:text-slate-400">{t.exitDate?.slice(0, 10)}</td>
    ...
    <div className="flex items-center justify-between text-sm">
      ...
      <span
        className={`font-semibold ${
          positive ? 'text-emerald-600 dark:text-emerald-400' : negative ? 'text-red-500 dark:text-red-400' : 'text-slate-900 dark:text-slate-100'
        }`}
      >
        {value}
      </span>
...
function StatRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between border-b border-slate-100 dark:border-surface-800 py-2 text-sm">
      <span className="text-slate-500 dark:text-slate-400">{label}</span>
