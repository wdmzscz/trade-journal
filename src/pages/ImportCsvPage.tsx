  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5 sm:space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Link
          to="/ibkr-sync"
          className="inline-flex items-center gap-2 rounded-lg border border-brand-200 dark:border-brand-800 bg-brand-50 dark:bg-brand-950/40 px-3 py-2 text-sm font-medium text-brand-700 dark:text-brand-300 hover:bg-brand-100"
        >
      ...
      <StorageInfo className="rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 p-4 shadow-sm" />

      {importResult && (
        <div className="flex items-start gap-3 rounded-xl border border-emerald-200 bg-emerald-50 dark:bg-emerald-950/40 p-4 text-emerald-800">
      ...
      <div className="grid gap-3 sm:grid-cols-2 sm:gap-4">
        <button
          ...
          className="flex items-center gap-3 rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 p-4 text-left shadow-sm hover:bg-slate-50 dark:hover:bg-surface-800 dark:bg-surface-800"
        >
          <div className="rounded-lg bg-brand-50 dark:bg-brand-950/40 p-2 text-brand-600 dark:text-brand-400">
            <Download className="h-5 w-5" />
          </div>
          <div>
            <p className="font-semibold text-slate-900 dark:text-slate-100">下载 CSV 模板</p>
            <p className="text-xs text-slate-500 dark:text-slate-400">TradeZella 通用格式模板</p>
          </div>
        </button>

        <div className="flex items-center gap-3 rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 p-4 shadow-sm">
          <div className="rounded-lg bg-slate-100 dark:bg-surface-800 p-2 text-slate-600 dark:text-slate-400">
            <FileText className="h-5 w-5" />
          </div>
          <div>
            <p className="font-semibold text-slate-900 dark:text-slate-100">券商账单自动识别</p>
            <p className="text-xs text-slate-500 dark:text-slate-400">IBKR / Fidelity 等格式自动配对开平仓</p>
          </div>
        </div>
      </div>
      ...
      <div
        ...
        className={`rounded-xl border-2 border-dashed p-8 text-center transition-colors sm:p-12 ${
          dragOver ? 'border-brand-500 bg-brand-50 dark:bg-brand-950/40' : 'border-slate-300 dark:border-surface-600 bg-white dark:bg-surface-900'
        }`}
      >
        <Upload className="mx-auto h-10 w-10 text-slate-400" />
        <p className="mt-3 text-sm font-medium text-slate-700 dark:text-slate-300">拖拽 CSV 文件到此处</p>
      ...
      {preview.length > 0 && (
        <div className="rounded-xl border border-slate-200 dark:border-surface-700 bg-white dark:bg-surface-900 p-4 shadow-sm">
          <p className="mb-3 text-sm font-semibold text-slate-800 dark:text-slate-200">导入方式</p>
          <div className="grid gap-2 sm:grid-cols-2">
            <label className={`flex cursor-pointer gap-3 rounded-xl border-2 p-3 ${importMode === 'merge' ? 'border-brand-500 bg-brand-50 dark:bg-brand-950/40' : 'border-slate-200 dark:border-surface-700'}`}>
              <input type="radio" name="importMode" checked={importMode === 'merge'} onChange={() => setImportMode('merge')} className="mt-1" />
              <div>
                <p className="text-sm font-medium text-slate-900 dark:text-slate-100">合并追加（推荐）</p>
                <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">保留旧记录，只添加新交易，自动跳过重复</p>
              </div>
            </label>
            <label className={`flex cursor-pointer gap-3 rounded-xl border-2 p-3 ${importMode === 'replace' ? 'border-brand-500 bg-brand-50 dark:bg-brand-950/40' : 'border-slate-200 dark:border-surface-700'}`}>
              <input type="radio" name="importMode" checked={importMode === 'replace'} onChange={() => setImportMode('replace')} className="mt-1" />
              <div>
                <p className="text-sm font-medium text-slate-900 dark:text-slate-100">完全替换</p>
                <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">删除该账户已有记录，用本次 CSV 覆盖</p>
              </div>
            </label>
          </div>
        </div>
      ...
      {preview.length > 0 && (
        <div className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-slate-100">预览 ({preview.length} 笔)</h3>
          ...
          <div className="overflow-hidden rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 shadow-sm">
              ...
              <table className="w-full min-w-[640px] text-sm">
                ...
                <tbody>
                  {preview.slice(0, 20).map((trade) => (
                    <tr key={trade.id} className="border-t border-slate-100 dark:border-surface-800">
                      <td className="px-3 py-2.5 font-semibold sm:px-4">{trade.symbol}</td>
                      <td className="px-3 py-2.5 sm:px-4">{trade.side === 'long' ? '做多' : '做空'}</td>
                      <td className="px-3 py-2.5 text-slate-600 dark:text-slate-400 sm:px-4">{trade.entryDate.slice(0, 10)}</td>
                      <td className="px-3 py-2.5 text-slate-600 dark:text-slate-400 sm:px-4">{trade.exitDate?.slice(0, 10) ?? '-'}</td>
      ...
      <div className="rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-900 p-4 shadow-sm sm:p-5">
        <h3 className="mb-3 font-semibold text-slate-900 dark:text-slate-100">数据存储说明</h3>
        <div className="space-y-3 text-sm text-slate-600 dark:text-slate-400">
          <p>数据保存在<strong className="text-slate-800 dark:text-slate-200">你当前使用的浏览器</strong>里（localStorage），不会自动同步到云端或其他设备。</p>
          <p>部署到 Vercel / GitHub Pages 等静态网站后，访问方式不变——仍是本地存储。换手机或换浏览器需要重新导入，或自行导出 CSV 备份。</p>
          <p>每月上传新 CSV 时，选择<strong className="text-slate-800 dark:text-slate-200">「合并追加」</strong>：旧记录保留，新交易追加，重复交易自动跳过。</p>
          <p>容量：一般浏览器限制约 5–10 MB，可存<strong className="text-slate-800 dark:text-slate-200">数万笔</strong>交易，日常使用足够。</p>
        </div>
      </div>
    </div>
  )
